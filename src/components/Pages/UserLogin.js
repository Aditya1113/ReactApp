...
export default function UserLogin() {
    ...
    useEffect(()=>{
        axios.get("https://test-api-users.onrender.com/users")
...
//   const [isLoggedIn, setIsLoggedIn] = useState(false);
  ...
  const handleLogin = () => {
    ...
    const matchedUser = users.find( (user) => user.email === loginDetails.username)
  ...
  return (
        ...
        <div className="container mt-4">
         ...
         Password
          ...
          <p>
          <button onClick={handleLogin} className="btn btn-sm btn-primary">Login</button>
          </p>
        </div>
  );
}
